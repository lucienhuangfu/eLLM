...
新增的[develop 设计处置台账](lift_index_merge_report.zh-CN.md#develop-原设计的删除替换与能力损失台账)逐项列出 18 类删除/替换/行为变化。特别需要补回的能力：max_tokens 限制、执行器 profiling、空 EOS 列表回退、完整模型对齐工具；并确认过载等待改拒绝、通知节奏和环境变量兼容的迁移策略。此前仅列出 lift_index 保留范围，不能据此推断 develop 所有功能也被保留。
