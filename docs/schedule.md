...
> 并将结果组织成 `prefill_list` 和 `decode_list` 两类切片结构，供 `ServingRunner` 和后续算子执行使用。

其中 `decode_list` 虽然名字里带 `decode`，但它实际上是本轮的扁平 token 视图：

* decode 轮时，它保存的是每条序列的单 token 切片
* prefill 轮时，它保存的是本轮 attention 需要处理的连续 token 区间
* `TopKSoftmax` 和 `LiftVector` 也会消费这份列表
...
| `last_token_flag` | 这段切片的最后一个 token 是否需要被下游当作“结果 token”处理 |
...
类型是 `DecodeList`，底层本质上是 `Vec<SequenceSlice>`。

* 它不是按线程拆开的，而是本轮全部 decode 或 prefill attention 切片的扁平列表
* decode 场景下每条序列本轮只会贡献 1 个长度为 1 的切片
* prefill 场景下，每条序列会贡献一段连续区间切片，长度可能大于 1
* `last_token_flag` 用来标记切片的最后一个 token 是否应该触发下游写回或结果提取
...
工作类型由 `plan_next_round()` 决定。

当前实现的判定规则是：

* 只要扫描到任意 `Phase::Decode`，本轮立即进入 `BatchPlan::Decode`
* 如果没有 decode，但存在至少一个 `Phase::Prefill`，则进入 `BatchPlan::Prefill`
* 如果两者都没有，则进入 `BatchPlan::Idle`
...
* 向 `decode_list` 追加一个长度为 1 的切片
* 这个切片的 `last_token_flag` 固定为 `true`

因此 decode 路径的特征非常清晰：

* 每条序列本轮只贡献 1 个 token
* `decode_list` 中的 decode 切片长度恒为 1
...
prefill 候选由 `plan_next_round()` 收集。
...
* 面向扁平 token 视图的连续区间，也就是写入 `decode_list`
* 面向线程执行的静态切片视图，也就是写入 `prefill_list`

这里有一个关键细节。对于单条序列，本轮真正能写入 `decode_list` 的长度不是固定等于 `remaining`，而是：
...
* `record.kv_index += scheduled_for_record`
...
同时，调度器会把该序列本轮实际调度到的最后一个 token 标成 `last_token_flag = true`。这个标记由后续 `TopKSoftmax` 和 `LiftVector` 识别，用来决定是否把该 token 的输出写回到序列结果区。

这意味着当前实现中：

* `sequence_index` 在调度阶段就会前移
* `kv_index` 也会在调度阶段同步前移
...
如果本轮是 Prefill:
    ...
    依次处理每条序列
        写入 decode_list 中本轮实际连续区间
        再按线程配额拆到 prefill_list
        把最后一个已调度 token 标成 last_token_flag = true
        立即推进 sequence_index、kv_index，并扣减 filling_length
...
* `prefill_count = 48`
* `decode_count = 8`
* `decode_list` 中有 8 个长度为 6 的切片；每条切片的 `last_token_flag` 都为 `true`
* `prefill_list` 中每个线程拿到 1 个长度为 6 的切片
...
* 两条序列各自本轮最后一个已调度 token 都会在 `decode_list` 中被标记为 `last_token_flag = true`
...
* `decode_list` 同时承担 decode token、prefill attention 和结果提取的扁平视图职责
* Prefill 场景下会通过 `last_token_flag` 标记本轮最后一个 token，供后续 `TopKSoftmax` 和 `LiftVector` 处理
* 调度阶段会直接推进 `sequence_index`、`kv_index`、扣减 `filling_length`，并在补齐后切换 `phase`

---

# 1️⃣1️⃣ 核心思想一句话

> 这套方案的核心，是先用 Decode 优先策略决定单轮工作类型；Prefill 轮把 token 静态切成 `SequenceSlice` 分配到各线程，decode 与 prefill attention 任务则统一保存在扁平 `decode_list` 中并在执行阶段再按 `assign` 公平切分。与此同时，Prefill 轮会在调度阶段直接推进序列游标、同步更新 `kv_index`、扣减剩余 `filling_length` 并更新阶段状态。
