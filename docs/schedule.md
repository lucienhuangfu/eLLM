...
> 并将结果组织成 `prefill_list`、`round_token_slices` 和 `attention_list` 三类切片结构，供 `ServingRunner` 和后续算子执行使用。
...
| `lift_index` | 额外的提升目标下标；当前实现里普通切片固定为 `0`，只有 prefill 轮额外写入 `round_token_slices` 的提升切片会把它设为目标 batch 槽位索引 |
...
* 再向 `round_token_slices` 追加一个长度为 1 的切片
...
* `round_token_slices` 只是把这些单 token 请求按出现顺序串成一段扁平列表
...
* 面向 `LiftVector` 的最后 token 提升视图，也就是写入 `round_token_slices`
...
* 在 `round_token_slices` 中额外记录这 2 个 token 里的最后一个 token，供后续提升到对应 batch 槽位
...
这个提升动作由后续 `LiftVector` 算子消费，用来把最后一个 token 的结果搬运到后续 decode 更容易访问的位置。当前实现只把这类提升任务追加到扁平 `round_token_slices` 中，具体由哪个线程执行同样在运行时通过 `assign` 决定。
...
如果本轮是 Prefill:
	...
	依次处理每条序列
		...
		再为最后一个已调度 token 生成一条 lift slice 到 round_token_slices，其中 lift_index = batch_index
...
* `round_token_slices` 中有 8 个长度为 1 的提升切片；每条切片的 `sequence_index` 指向该序列本轮最后一个 token，`lift_index` 等于对应 `batch_index`
...
* 两条序列各自本轮最后一个已调度 token 都会在 `round_token_slices` 中生成一条提升切片
...
* Prefill 场景下还会额外记录每条序列本轮最后一个 token 的提升位置到 `round_token_slices`，这类切片的 `lift_index` 等于 `batch_index`
...
> 这套方案的核心，是先用 Decode 优先策略决定单轮工作类型；Prefill 轮把 token 静态切成 `SequenceSlice` 分配到各线程，Decode 和 lift 任务则统一保存在扁平 `round_token_slices` 中并在执行阶段再按 `assign` 公平切分。与此同时，Prefill 轮会在调度阶段直接推进序列游标、扣减剩余 `filling_length` 并更新阶段状态。
