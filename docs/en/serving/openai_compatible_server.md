...
* `src/serving/parser.rs` — incremental streaming parser for model output
...
## 11. Output Parser (`parser.rs`)

`src/serving/parser.rs` implements an incremental streaming parser that interprets model text output — extracting plain content, reasoning traces, and tool calls — without ever re-scanning historical output.

### Design goals

* **Zero re-parsing** — only newly arrived deltas are processed; previously emitted events are never revisited.
* **Boundary-safe buffering** — a small suffix of the internal buffer is always retained when it could be the prefix of an upcoming tag, preventing split-tag misses.
* **Load-time rule selection** — format rules are chosen at construction time (from `ModelFamily`), not inferred at runtime.
...
Holds the four tag strings and the tool-call format for one model family:

| Field | Purpose |
|---|---|
| `tool_start` / `tool_end` | Opening / closing tag that wraps a tool invocation |
| `think_start` / `think_end` | Opening / closing tag for chain-of-thought reasoning |
| `tool_format` | How to interpret the payload inside the tool block |

Built-in presets and the families they target:

| Preset | `ModelFamily` | Tool format |
|---|---|---|
| `qwen()` | `Qwen` | `Tagged` |
| `deepseek()` | — | `Tagged` |
| `hermes()` | fallback / `Unknown` | `Tagged` |
| `llama3_json()` | `Llama` | `RawJson` |
| `mistral()` | `Mixtral` | `PrefixedJson` |
| `minimax_m1()` | `MiniMax`, `MiniMaxM2` | `Tagged` |
| `minimax_m2()` | — | `MiniMaxM2` |

`ParserRule::for_model_family(family)` selects the correct preset automatically.

#### `ToolCallFormat`

Controls how the JSON payload inside a tool block is decoded:

* **`Tagged`** — the block contains a JSON object (or array) directly; parsed with `serde_json`.
* **`PrefixedJson`** — Mistral style: `[TOOL_CALLS] FunctionName { ... }` — the function name precedes the JSON body.
* **`RawJson`** — Llama 3 style: raw JSON object following the `<|python_tag|>` sentinel; no closing tag.
* **`MiniMaxM2`** — XML-like `<invoke name="…"><parameter name="…">value</parameter></invoke>` syntax.

#### `ParserOptions`

Wraps a `ParserRule` together with two feature flags:

* `reasoning_parser` — when `false`, `<think>…</think>` tags are passed through as plain `Content`.
* `tool_call_parser` — when `false`, tool-call tags are passed through as plain `Content`.

Both flags default to `true` in `ParserOptions::new(rule)`.

#### `ParserEvent`

The output of each `feed()` call is a `Vec<ParserEvent>`:

| Variant | Meaning |
|---|---|
| `Content(String)` | Plain assistant text to show to the user |
| `Reasoning(String)` | Chain-of-thought fragment (hidden or displayed separately) |
| `ToolCallDelta(ToolCallDelta)` | Raw JSON fragment of an in-progress tool call |
| `ToolCall(ToolCall)` | Completed, fully parsed tool call (`name` + `arguments`) |
| `Finish` | (reserved) end-of-stream signal |

`ToolCall` carries a `name: String` and `arguments: serde_json::Value`.

---

### State machine

`IncrementalStreamingParser` drives a four-state machine:

```
Normal  ──think_start──►  Reasoning  ──think_end──►  Normal
Normal  ──tool_start───►  ToolCall   ──(auto)──────►  ToolJson
ToolJson ──tool_end────►  Normal
```

| State | What happens on each `feed()` |
|---|---|
| `Normal` | Emit `Content` up to the next marker; buffer the tail that could be a partial tag |
| `Reasoning` | Emit `Reasoning` fragments until `think_end` is found |
| `ToolCall` | Transition marker consumed; immediately advances to `ToolJson` |
| `ToolJson` | Accumulate JSON; emit `ToolCallDelta` fragments; emit `ToolCall` when complete |

The helper `longest_suffix_prefix_len()` computes how many trailing bytes of the buffer must be withheld because they are a prefix of a marker string. This is what makes the parser boundary-safe without buffering the entire stream.

---

### Tool-call payload parsing

Four internal strategies correspond to the four `ToolCallFormat` variants:

**`Tagged` / `MiniMaxM2`**  
Accumulates all bytes between `tool_start` and `tool_end`. On `tool_end`:
- `Tagged`: calls `serde_json::from_str` and `extract_tool_call_value` to support both flat (`{name, arguments}`) and nested (`{function: {name, arguments}}`) object shapes, as well as top-level JSON arrays of calls.
- `MiniMaxM2`: calls `parse_minimax_m2_tool_call`, which walks the XML-like `<invoke>` / `<parameter>` structure with string scanning (no XML parser dependency).

**`RawJson`**  
Triggered by `<|python_tag|>` (no closing tag). Buffers all subsequent text and uses `extract_complete_json_prefix_range` to detect when a complete JSON value has arrived. The remainder after the JSON object is pushed back into the main buffer and parsing continues in `Normal` state.

**`PrefixedJson`**  
Triggered by `[TOOL_CALLS]`. Expects `FunctionName { … }` — the function name is extracted as the text before the first `{`, the JSON body is extracted by `extract_complete_json_prefix_range`, and both are combined into a `ToolCall`.

---

### `parser_event_stream`

A convenience adapter that wraps a `StreamingParser` + a `Stream<Item = TextDelta>` into a `Stream<Item = ParserEvent>`, feeding each delta into the parser and yielding the resulting events. Intended for use in the streaming response path of `chat_handlers`.
...
### Disabling parsers at runtime

Both the reasoning and tool-call parsers can be turned off independently:

```rust
let options = ParserOptions {
    rule: ParserRule::qwen(),
    reasoning_parser: false,  // <think> tags pass through as Content
    tool_call_parser: false,  // <tool_call> tags pass through as Content
};
let mut parser = IncrementalStreamingParser::with_options(options);
```

This is useful when a downstream consumer needs the raw model output, or when the deployed model does not use standard tags.
