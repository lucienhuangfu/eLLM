...
| `last_token_flag` | Whether the last token should be treated as the result token |
...
* `decode_list` is a flattened global slice list

---

## 2. Single-Round Scheduling Entry
...
* `decode_list` is just a list of single-token slices
...
## 5. What `decode_list` Really Means

`decode_list` is not a list "for decode only".

In both modes it carries the flattened token view:
