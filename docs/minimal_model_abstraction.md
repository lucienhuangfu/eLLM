...
这份方案的目标不是一次性把当前 `src/transformer` 重构成完整框架，而是在**尽量少改现有代码**的前提下，把当前实现从“Qwen3-MoE 专用路径”提升为“可扩展到 dense / sparse MoE / hybrid attention 的通用 decoder 路径”。
...
当前阶段不建议直接把 `src/transformer` 改成完整的 `src/model/core/components/families/spec` 目录树。
...
1. 不要立即把 `src/transformer` 改名成 `src/model`
