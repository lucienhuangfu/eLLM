...
fn main() {
    let model_dir = std::env::args().nth(1).unwrap_or_else(|| "models/Qwen3-0.6B".to_string());
    let max_tokens: usize = std::env::args().nth(2).unwrap_or_else(|| "6".to_string()).parse().unwrap_or(6);

    let config = Config::load_from_file(format!("{model_dir}/config.json")).unwrap();
    let gen_cfg = GenerationConfig::load_from_file(format!("{model_dir}/generation_config.json")).ok();
    ...
    let chat_template = ChatTemplate::from_model_files(&chat_template_path, &tokenizer_config_path).ok().unwrap();
    let tokenizer = load_tiktoken(&tokenizer_path, &tokenizer_config_path).unwrap();

    let messages_batch = [
        [("user", "Tell me a short joke about programming.")],
        [("user", "What is the difference between stack and heap memory?")],
    ];
    ...
    let params = SafeTensorsLoader::new(&model_dir).unwrap().load_all_weights_f16().unwrap();
    ...
    let eos_ids = gen_cfg.as_ref().and_then(GenerationConfig::eos_token_ids)
        .filter(|ids| !ids.is_empty()).unwrap_or(config.eos_token_ids);
    ...
    for (batch_idx, ids) in all_input_ids.iter().enumerate() {
        for (pos, token_id) in ids.iter().enumerate() {
            unsafe { *sequences.as_mut_ptr().add(batch_idx * sequence_length + pos) = *token_id as usize; }
        }
    }
    ...
    let phase2_start = operator_queue.iter()
    ...
    for batch_idx in 0..batch_size {
        ...
        let slice = SequenceSlice {
            batch_index: batch_idx, sequence_index: 0,
            token_start_index: token_offset, length: input_len, last_token_flag: true,
        };
    ...
    f16::with_global(|pool| {
        ...
        for batch_idx in 0..batch_size {
            let input_len = all_input_ids[batch_idx].len();
            unsafe { std::ptr::copy(norm_ptr.add((offset + input_len - 1) * 1024), norm_ptr.add(batch_idx * 1024), 1024); }
            offset += input_len;
        }
    });
    ...
    for batch_idx in 0..batch_size {
        ...
        println!("Batch {batch_idx} Token 1: {tok} ({})",
    ...
    for _step in 1..max_tokens {
        ...
        if bl_ref.iter().all(|s| matches!(s.phase, Phase::Eos)) { break; }

        let mut step_slices = Vec::new();
        for batch_idx in 0..batch_size {
            if matches!(bl_ref[batch_idx].phase, Phase::Eos) { continue; }
            step_slices.push(SequenceSlice {
                batch_index: batch_idx,
                sequence_index: bl_ref[batch_idx].sequence_index,
                token_start_index: batch_idx, length: 1, last_token_flag: true,
            });
        }
        ...
        for batch_idx in 0..batch_size {
            if matches!(bl_ref[batch_idx].phase, Phase::Eos) { continue; }
            let tok = unsafe { *seq_ptr.add(batch_idx * sequence_length + bl_ref[batch_idx].sequence_index) };
            generated_tokens[batch_idx].push(tok);
        }
    }
    ...
    for (batch_idx, tokens) in generated_tokens.iter().enumerate() {
        let text = tokenizer.decode(tokens.iter().map(|t| *t as u32).collect::<Vec<_>>()).unwrap_or_default();
