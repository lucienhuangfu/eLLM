...
fn test_write_and_read_tokens() {
    ...
    let mut batch = BatchSequence::<f32> {
        ...
        row_size: batch_size,
        col_size: seq_len,
        tokenizer: BatchSequence::<f32>::default().tokenizer,
        chat_template: BatchSequence::<f32>::default().chat_template,
    };

    let tokens: Vec<u32> = vec![10, 20, 30, 40, 50];
    let written = batch.write_tokens(0, &tokens, 0.7).unwrap();
    assert_eq!(written, 5);
...
fn test_write_tokens_at_offset() {
    ...
    batch.write_tokens(0, &prefix, 1.0).unwrap();
...
fn test_write_tokens_respects_col_size_limit() {
    ...
    let written = batch.write_tokens(0, &tokens, 1.0).unwrap();
    assert_eq!(written, 8);
}
...
async fn test_get_prefix_match_len_partial_prefix_match() {
    ...
    let prefix_len = manager.get_prefix_match_len(session_id, &new_tokens).await;
...
async fn test_get_prefix_match_len_no_prefix_match() {
    ...
    let delta = manager.get_prefix_match_len(session_id, &new_tokens).await;
...
async fn test_get_prefix_match_len_exact_match() {
    ...
    let prefix_len = manager.get_prefix_match_len(session_id, &tokens).await;
    assert!(prefix_len.is_some());
    assert_eq!(prefix_len.unwrap(), 5);
}
...
async fn test_get_prefix_match_len_zero_token_count() {
    ...
    let delta = manager.get_prefix_match_len("zero_token", &[1, 2, 3]).await;
    assert!(delta.is_none());
}
