...
use crate::runtime::session::{Phase, SessionMode, SessionHandle};
...
async fn test_acquire_release_reuse_reusable() {
    ...
    let token_count = manager.with_slots(|slots| slots[slot1].sequence_index);
    manager.release_session("user_a", token_count).await;
...
async fn test_non_reusable_releases_immediately() {
    ...
    manager.release_session("s1", 10).await;
...
async fn test_slot_reclaimed_after_timeout() {
    ...
    let slot1 = h1.slot_index;
    manager.release_session("timeout_user", 10).await;
    ...
    let h4 = manager.acquire_session("timeout_user").await.unwrap();
    assert!(!h4.is_reused);
}
...
async fn test_resumed_session_cancels_timeout() {
    ...
    manager.with_slots_mut(|slots| {
        slots[slot1] = crate::runtime::session::SlotState::new_decode_state(10, 10);
    });
...
async fn test_with_slots_access_patterns() {
    ...
    manager.with_slots_mut(|slots| {
        slots[0] = crate::runtime::session::SlotState::new_prefill_state(0, 10);
        slots[1] = crate::runtime::session::SlotState::new_decode_state(5, 5);
    });
