...
fn main() {
    ...
    let gen_cfg = GenerationConfig::load_from_file(format!("{}/generation_config.json", model_dir)).ok();
    ...
    for prompt in &prompts {
        let rendered = chat_template.apply_chat_template(&[("user", *prompt)], true).unwrap();
    ...
    let mut batch_seq = BatchSequence::<f16>::new(
        sequences_ptr, batch_size, sequence_length,
        &tokenizer_path, &tokenizer_config_path, &chat_template_path,
    ).unwrap();

    let mut written_lengths = Vec::new();
    for (slot, prompt) in prompts.iter().enumerate().take(batch_size) {
        let write_len = batch_seq.write_prompts(slot, &[("user", prompt)], 1.0).unwrap();
    ...
    let batch_list: Vec<SequenceState> = written_lengths.iter().map(|&len| SequenceState {
        filling_length: len, sequence_index: 0, kv_index: 0,
        phase: Phase::Prefill,
        notify: Arc::new(tokio::sync::Notify::new()),
    }).collect();

    let batch_scheduler = BatchScheduler::new(sequence_length, batch_size, thread_num);
    batch_scheduler.batch_list.with_mut(|list| *list = batch_list);
    ...
    batch_list_ref.with(|list| {
        for (slot, record) in list.iter().enumerate() {
            ...
            let gen_end = record.kv_index.min(input_len + 32.min(sequence_length - input_len));
