...
fn build_linear(cache: Rc<RefCell<Cache<f16>>>, operator_queue: Rc<RefCell<Vec<Operator<f16>>>>) {
    let head_size = 128;
    let head_num = 32;
...
fn main() {
    ...
    let mut sequences: Vec<usize> =
        vec![0; (config.max_position_embeddings + 1) * config.batch_size];
