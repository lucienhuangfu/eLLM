...
fn main() {
    if !cfg!(target_arch = "x86_64") || !std::arch::is_x86_feature_detected!("avx512fp16") {
        println!("AVX512FP16 not supported on this machine, skipping matmul topk perf run.");
        return;
    }

    println!(
        "matmul topk perf init: seq_len={}, seq_chunk={}, batch={}, padded_batch={}, n={}, k={}, topk={}",
        SEQUENCE_LENGTH, SEQUENCE_CHUNK_SIZE, BATCH_SIZE, PADDED_BATCH_SIZE, N, K, TOPK
    );
    ...
    let input_tensor = Tensor::<f16>::from_cache(
        ...
        "model.layers.0.perf.matmul_topk.input".to_string(),
        cache.clone(),
    ...
    let weight_tensor = Tensor::<f16>::from_cache(
        vec![N, K],
        "model.layers.0.perf.matmul_topk.weight".to_string(),
        cache.clone(),
        operator_queue.clone(),
    );
    ...
    let _topk_output = input_tensor.matmul_local_topk(
        &weight_tensor,
        params,
        TOPK,
        "model.layers.0.perf.matmul_topk".to_string(),
    );

    assert_eq!(
        operator_queue.borrow().len(),
        1,
        "expected exactly one operator from matmul_local_topk"
    );
    ...
    println!(
        "matmul topk perf: batch={}, n={}, k={}, topk={}, threads={}, elapsed={:?}, effective_gflops={:.2}",
        BATCH_SIZE,
        N,
        K,
        TOPK,
