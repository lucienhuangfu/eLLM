    ...
    T: Copy + Add<Output = T> + Mul<Output = T> + Default,
{
    ...
    unsafe fn slot_of(&self, b: usize, e: usize) -> usize {
        ...
        // 为了安全，返回 0；你也可以改成 debug_assert! 并返回 0。
        0
    }
...
where
    T: Copy + Add<Output = T> + Mul<Output = T> + Default,
{
    // compute1: GEMM micro-kernel，把 A_tile×B_panel 累加到 acc
    default fn compute1(
        &self,
        _a_tile: *const T,
        _b_panel: *const T,
        ...
        // 默认空实现（实际 f16 专用内核在别处特化）
    }

    // compute2: 对一行 acc_row 做缩放并加到 out_row 上：
    //   out_row[j] += acc_row[j] * factor
    default fn compute2(
        &self,
        _out_row: *mut T,
        _acc_row: *const T,
