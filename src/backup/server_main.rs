...
pub async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let chat_template = Arc::new(
        std::fs::read_to_string("models/Qwen3-Coder-30B-A3B-Instruct/chat_template.jinja")
            .map_err(|e| format!("加载 chat_template.jinja 失败: {}", e))?,
    );
    println!("已加载 tokenizer: {}", tokenizer_path);
    println!("已加载 chat_template: models/Qwen3-Coder-30B-A3B-Instruct/chat_template.jinja");
