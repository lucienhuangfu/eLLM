...
#[inline]
unsafe fn compute_row_attention<T>(
    q_row_ptr: *const T,
    output_row_ptr: *mut T,
        ...
        + Exp,
{
    for index in 0..head_size {
        *output_row_ptr.add(index) = T::default();
    }

    let mut running_max = T::neg_infinity();
    let mut running_denom = T::default();

    for col_begin in (0..visible_col_end).step_by(col_step) {
        let block_end = (col_begin + col_step).min(visible_col_end);
        let block_len = block_end - col_begin;
        ...
        let next_max = if block_max > running_max {
            block_max
        } else {
            running_max
        };

        let carry = running_denom * (running_max - next_max).exp();
...
{
    unsafe {
        let col_step = col_size.max(1);
        let mut scores = vec![T::default(); col_step];

        for row in row_begin..row_end {
            let position = sequence_index + row;
            let visible_col_end = (position + 1).min(col_end);
            if visible_col_end == 0 {
                continue;
            }

            let row_offset = row * head_size;
            let q_row_ptr = q_head_ptr.add(row_offset);
            let output_row_ptr = output_head_ptr.add(row_offset);
