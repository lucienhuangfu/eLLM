...
mod tests {
    ...
    fn test_broadcast_microkernel_3x32_k64_ones() {
        ...
        unsafe {
            ...
            // 上层“默认对齐”的版本：用 Vec 分配 A/B/C
            let a_vec = vec![f16_from_f32(1.0); MR * LDA];
            let b_vec = vec![f16_from_f32(1.0); KC * NR];
            let mut c_vec = vec![f16_from_f32(0.0); MR * LDC];

            let a_ptr = a_vec.as_ptr();
            let b_ptr = b_vec.as_ptr();
            let c_ptr = c_vec.as_mut_ptr();
