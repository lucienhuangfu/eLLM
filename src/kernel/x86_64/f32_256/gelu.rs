use std::ptr;
use std::arch::x86_64::*;
// use super::super::SQRT_2_DIV_PI;
const SQRT_2_DIV_PI : f32 = (0.7978845608028654f32);
use super::math::tanh256;

#[inline]
fn _single_gelu(x: f32) -> f32 {
    let y = (x + 0.044715f32 * x * x * x) * SQRT_2_DIV_PI;
    let y2 = 0.5f32 * x * (1.0f32 + y.tanh());
    y2
}

pub fn _gelu(input_ptr: *const f32, output_ptr: *mut f32, length: usize) {
    unsafe {
        let rem = length % 8;
        let length2 = length - rem;

        if rem != length {
            let var1 = _mm256_set1_ps(0.044715f32);
            let var2 = _mm256_set1_ps(SQRT_2_DIV_PI);
            let var10 = _mm256_set1_ps(0.5f32);
            let var_one = _mm256_set1_ps(1.0f32);
            for (ptr1, ptr2) in (0..length2).step_by(8).map(|x| (input_ptr.add(x), output_ptr.add(x))) {
                let x = _mm256_loadu_ps(ptr1);
                let mut y = _mm256_mul_ps(x, x);
                y = _mm256_mul_ps(y, x);
                y = _mm256_fmadd_ps(y, var1, x);
                y = _mm256_mul_ps(y, var2);
                y = tanh256(y);
                y = _mm256_add_ps(y, var_one);
                y = _mm256_mul_ps(y, x);
